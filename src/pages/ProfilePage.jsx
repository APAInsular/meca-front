...
const ProfilePage = () => {
    ...
    const [userInfo, setUserInfo] = useState(null);  // Inicializar como null
    const navigate = useNavigate();

    useEffect(() => {
        const fetchUserProfile = async () => {
            try {
                const response = await axios.get(`${baseUrl}/user/${userId}/profile`);
                if (response.data) setUserInfo(response.data);  // Asignar response.data
    ...
    if (!userInfo) {
        ...
        return (
            <div>
                ...
                <div className="mt-4">
                    {activeTab === "info" ? (
                        <UserInfoProfile userInfo={userInfo} />
                    ) : activeTab === "favorite" ? (
                        <UserFavoriteProfile saves={userInfo.saves} />
                    ) : activeTab === "avatar" ? (
                        <UserAvatarProfile avatar={userInfo.avatar} />
                    ) : activeTab === "publications" ? (
                        <UserPublicationProfile blogEntries={userInfo.blogEntries} />
                    ) : (
                        null
                    )}
                </div>
